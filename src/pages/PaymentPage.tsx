...
//   // Load recipient info saved from Checkout Details
...
//     // Minimal payment validation (UI-only)
//     if (!cardName.trim()) return setErr("Please enter name on card.");
//     if (cardNumber.replace(/\s/g, "").length < 12)
//       return setErr("Please enter a valid card number.");
//     if (!exp.trim()) return setErr("Please enter expiry.");
//     if (cvv.trim().length < 3) return setErr("Please enter CVV.");
...
//       // 2) Mark paid (this moves to FULFILLING if supplier jobs exist)
//       const paid = await apiPost<OrderOut>(
//         `/api/orders/${created.order.id}/mark-paid`,
//         {},
//       );

//       // 3) Clear cart & meta, redirect success
//       cart.clear();
//       try {
//         localStorage.removeItem("bg_checkout_meta");
//       } catch {}

//       nav(`/order-success/${paid.id}`);
//     } catch (e: any) {
...
type OrderCreatePayload = {
  ...
  items: Array<{
    product_id: number;
    qty: number;
    delivery_date?: string | null; //  NEW (YYYY-MM-DD)
  }>;
};
...
export default function PaymentPage() {
  ...
  async function handlePayNow() {
    ...
    // ✅ Delivery date should already exist (PDP enforced), but validate anyway:
    ...
    const payload: OrderCreatePayload = {
      ...
      items: cart.items.map((it) => ({
        product_id: it.product_id,
        qty: it.qty,
        delivery_date: it.delivery_date ?? null, // ✅ NEW
      })),
    };
